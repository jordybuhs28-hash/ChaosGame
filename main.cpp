...
sf::Color getShapeColor(int sides) {
	switch(sides) {
		case 3: return sf::Color::Green; //Triangle and so on following above pattern
		case 4: return sf::Color::Blue;
		case 5: return sf::Color::Magenta;
		case 6: return sf::Color::Cyan;
		case 7: return sf::Color::Yellow;
		case 8: return sf::Color(255, 165, 0);
		case 9: return sf::Color::Red;
		case 10: return sf::Color::White;
		defualt: return sf::Color::Green;
	}
	//Safety fallback (silence warning)
...
//	sf::RenderWindow window(desktop, "Chaos Game", sf::Style::Fullscreen);
	...
	sf::Text legend ("", font, 18);
	legend.setFillColor(sf::Color::White);
	legend.setPosition(10, 40);
			...
			if (event.type == sf::Event::MouseButtonPressed) {
				...
				if (!started) {
					if (event.mouseButton.button == sf::Mouse::Left) {
						...
						legend.setString(getShapeName(vertices.size()) + " fractal (" + std::to_string(vertices.size()) + " sides)");
					}
				}
			}
...
//	}
		...
		// draw chaos points
		sf::Color fractalColor = getShapeColor(vertices.size());
		for (auto &p : points) {
			sf::CircleShape dot(2);
			dot.setFillColor(fractalColor);
			dot.setPosition(p);
			window.draw(dot);
		}
